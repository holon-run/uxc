...
2. Configure credential/binding for repeatable auth:
   ...
   - If the runtime already injects `OKX_ACCESS_KEY` into the `uxc` daemon environment, `uxc auth credential set okx-mcp --auth-type api_key --api-key-header OK-ACCESS-KEY --secret-env OKX_ACCESS_KEY` also works.
...
- If call result contains `Request header OK-ACCESS-KEY can not be empty`:
  ...
  - update credential with explicit header: `uxc auth credential set okx-mcp --auth-type api_key --api-key-header OK-ACCESS-KEY --secret "$OKX_ACCESS_KEY"`
