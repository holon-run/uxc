...
jq -e '.paths["/api/protocols"] and .paths["/yields/pools"]' "${SCHEMA_FILE}" >/dev/null 2>&1 || fail 'OpenAPI schema missing expected DefiLlama paths'
...
rg -q 'uxc link defillama-openapi-cli https://pro-api.llama.fi --schema-url ' "${SKILL_FILE}" || fail 'missing fixed link create command with schema-url'
rg -q 'defillama-openapi-cli -h' "${SKILL_FILE}" || fail 'missing help-first host discovery example'
rg -q 'defillama-openapi-cli get:/api/protocols -h' "${SKILL_FILE}" || fail 'missing operation-level help example'
