...
async fn execute_cli(cli: &Cli) -> Result<OutputEnvelope> {
    ...
    }) = &cli.command
    {
        ...
        return handle_link_command(name, host, dir.as_deref(), *force, &exclusive).await;
    }
...
fn help_data_for_path(path: &[&str]) -> HelpData {
    match path {
        ...
        ["link"] => HelpData {
            ...
            usage:
                "uxc link <name> <host> [--dir <dir>] [--daemon-exclusive <key> ...] [--force]"
                    .to_string(),
            commands: vec![],
            notes: vec![
...
async fn handle_link_command(
    ...
    let launcher = build_link_launcher(name, host, daemon_exclusive);
...
fn build_link_launcher(name: &str, host: &str, daemon_exclusive: &[String]) -> String {
...
fn looks_like_uxc_link_launcher(content: &[u8]) -> bool {
    ...
    let mut unix_like = false;
    for line in text.lines().take(10) {
        let trimmed = line.trim_start();
        if trimmed.starts_with("UXC_LINK_NAME='") && trimmed.contains(" exec uxc ") {
            unix_like = true;
            break;
        }
    }
